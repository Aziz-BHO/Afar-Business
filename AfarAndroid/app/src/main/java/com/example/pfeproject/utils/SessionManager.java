...
//                                exp rest = (20 000 - 15 000) + 7 000
                                ...
                                if (Integer.parseInt(totalPoint.getTotalpoints()) >= Integer.parseInt(entTotalPoint.getTotalpoints())) {
                                    ...
                                    userRestPoint = difftTotal > entTotalPoint.getRestpoints() ? entTotalPoint.getRestpoints() : difftTotal; //difftTotal+ ;
...
//                update cmd total point
                    ...
                    Log.d(TAG, "checkAndSaveUserCommand: check if product is saved correctly ... => " + (myCommand.getProduct() != null));
                    ...
                    if (cmd.getStatus().equals(Types.Cancled) && (retrieveCommandPositionById(cmd.getId()) != -1)) {
                        int cmdPoint = cmd.getProduct().getDiscountPoints();
                        String entrId = cmd.getProduct().getEntreprise().getId();

                        TotalPoint oldUserTotalPoint = retrieveUserEntreprisePoint(entrId);
                        oldUserTotalPoint.setRestpoints(oldUserTotalPoint.getRestpoints() + cmdPoint);
                        Log.i(TAG, "checkAndSaveUserCommand: after update oldUserTotalPoint the new userRestpoint = " + oldUserTotalPoint.getRestpoints());
                        userTotalPoints.set(retrieveUserEntreprisePointPositionByEntId(entrId), oldUserTotalPoint);
                        saveOnlyUserPoint(userTotalPoints);
                        for (Command c : savedCommand)
                            c.setTotalPointsPerEntreprise(userTotalPoints);

                        savedCommand.remove(retrieveCommandPositionById(cmd.getId()));
                        Log.i(TAG, "checkAndSaveUserCommand: commmand removed ...");
                    } else if (cmd.getStatus().equals(Types.Confirmed) && (retrieveCommandPositionById(cmd.getId()) != -1))
                    {
                        savedCommand.remove(retrieveCommandPositionById(cmd.getId()));
                    }
    ...
    public TotalPoint retrieveUserPointFromLastCommandByEntrId(String entreprise_id) {
        ...
        Log.i(TAG, "retrieveUserPointFromLastCommandByEntrId: Rpoint = " + point.getRestpoints()+" Tpoint = "+point.getTotalpoints());
        return point;
    }
    ...
    public void deleteUnusedCommand(Command[] userCommand) {
        ...
        if (saveCmd.size() > 0) {
            ...
            for (Command userCmd : userCommand) {
                ...
                if (userCmd.getStatus().equals(Types.Cancled) && (retrieveCommandPositionById(userCmd.getId()) != -1)) {
                    int cmdPoint = userCmd.getProduct().getDiscountPoints();
                    String entrId = userCmd.getProduct().getEntreprise().getId();

                    TotalPoint oldUserTotalPoint = retrieveUserEntreprisePoint(entrId);
                    oldUserTotalPoint.setRestpoints(oldUserTotalPoint.getRestpoints() + cmdPoint);
                    Log.i(TAG, "deleteUnusedCommand: after update oldUserTotalPoint the new userRestpoint = " + oldUserTotalPoint.getRestpoints());

                    ArrayList<TotalPoint> userTotalPoints = retrieveUserPoints();
                    userTotalPoints.set(retrieveUserEntreprisePointPositionByEntId(entrId), oldUserTotalPoint);
                    saveOnlyUserPoint(userTotalPoints);
                    for (Command c : saveCmd)
                        c.setTotalPointsPerEntreprise(userTotalPoints);

                    saveCmd.remove(retrieveCommandPositionById(userCmd.getId()));
                } else if (userCmd.getStatus().equals(Types.Confirmed) && (retrieveCommandPositionById(userCmd.getId()) != -1))
                {
                    saveCmd.remove(retrieveCommandPositionById(userCmd.getId()));
                }
            }
...
//        else remove confirmed or cancled cmd
            if (userCommand.length > 0) {
                ...
                saveUserCommandStat(saveCmd);
                Log.d(TAG, "deleteUnusedCommand: save new command .... size => " + saveCmd.size());
            }
    }
